...
def prepare_data_for_feature_engineering(shots_and_goals, other_events):
    season_data = pd.concat([shots_and_goals, other_events])
...
def create_preceding_event_table(season_data):
    preceding_event = season_data.loc[:,
                      ["game_seconds", "which_period", "event_type", "coord_x", "coord_y", "angle_from_net"]]
    ...
    preceding_event = preceding_event.rename(
        columns={"game_seconds": "pre_game_seconds", "which_period": "pre_which_period", "event_type": "pre_event_type",
